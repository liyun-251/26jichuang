                                ...
                                unsigned char* pcm_bytes, int num_bytes)
{
    ...
    for (cycle = search_start; cycle < fsx_depth * 8 && byte_idx < num_bytes; cycle++) {
        byte_ofs = cycle / 8; //该周期（cycle）是失效数据中的第几个字节
        bit_ofs = cycle % 8; //该周期（cycle）是失效数据中的第几个 bit

        /* FSX=1 → 失效位=1（图形期望 0，实际输出 1，不匹配） */
        if ((fsx_fail_bits[byte_ofs] >> bit_ofs) & 1) {//提取第byte_ofs字节的第bit_ofs位，判断是否为1，即FSX=1
            ...
            for (bit = 0; bit < 8; bit++) {
                ...
                dx_byte_ofs = dx_cycle / 8; //第几个dx字节
                dx_bit_ofs  = dx_cycle % 8; //dx_byte_ofs个dx字节中的第几个dx位
                dx_level    = (dx_fail_bits[dx_byte_ofs] >> dx_bit_ofs) & 1; ////提取第byte_ofs字节的第bit_ofs位，判断是否为1，即dx=1
                pcm_byte |= (unsigned char)(dx_level << (7 - bit));  // MSB-first 组装pcm字节
            }
